...
select account_id, month(day) as month, sum(if( type='Creditor', amount, 0)) as total_m_income
from Transactions 
group by account_id, month ), 
temp_table as (
select t.account_id, t.month  from total_income t
  join   Accounts a
 on t.account_id = a.account_id
 where t.total_m_income> a.max_income ) 

select distinct t1.account_id from temp_table t1 join temp_table t2
on t1.account_id = t2.account_id and t1.month - t2.month =1
