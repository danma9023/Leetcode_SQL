...
select s.gender, s.day, sum(s1.score_points) as total
from Scores s left join Scores s1
on s.gender = s1.gender
and s.day >= s1.day
...
# Write your MySQL query statement below
select s.gender, s.day, sum(s.score_points) over(partition by s.gender order by s.day) total
from Scores s
