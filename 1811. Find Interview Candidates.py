...
with gold_count as (
select c.gold_medal as user_id from Contests c 
group by c.gold_medal
having count(c.gold_medal) >= 3),
all_users as (select u.user_id, c.contest_id from Contests c
             left join Users u
             on u.user_id = c.gold_medal
             or u.user_id = c.silver_medal
             or u.user_id = c.bronze_medal),
consecutive as (select a1.user_id, a1.contest_id, count(*) as cnt from all_users a1
               join all_users a2 
               on a1.user_id=a2.user_id
               and a1.contest_id between a2.contest_id and a2.contest_id+2
               group by a1.user_id,a1.contest_id
               having cnt >=3), 
qualified as (select g.user_id from gold_count g
             union 
             select c.user_id from consecutive c)
select u.name, u.mail from qualified q 
left join Users u
