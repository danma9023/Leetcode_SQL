...
with avg_table as (select order_id, avg(quantity) as avg_q
from OrdersDetails 
group by order_id),
max_table as 
...
select m.order_id from max_table m
where m.max_q > all (select avg_q from avg_table)
group by m.order_id
...
select m.order_id from max_table m
where m.max_q >  (select max(avg_q) from avg_table)
group by m.order_id
