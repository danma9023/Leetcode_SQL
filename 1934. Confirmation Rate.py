...
with temp_t as ( 
select user_id, sum(case when action = 'confirmed' then 1 end) / count(action) as rate
from Confirmations 
group by user_id )

select s.user_id, round(ifnull(t.rate, 0), 2) as confirmation_rate from Signups s
left join temp_t t 
